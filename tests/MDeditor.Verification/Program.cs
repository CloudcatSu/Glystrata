...
using MDeditor.Core.Groups;
using MDeditor.Core.Layout;
using MDeditor.Core.Markdown;
using MDeditor.Core.Persistence;
using MDeditor.Core.Snapshots;

namespace MDeditor.Verification;

internal static class Program
{
    public static async Task<int> Main()
    {
        var root = Path.Combine(Path.GetTempPath(), $"MDeditor.Verification-{Guid.NewGuid():N}");
        ...
        try
        {
            ...
            Console.WriteLine("All MDeditor verification assertions passed.");
            return 0;
        }
