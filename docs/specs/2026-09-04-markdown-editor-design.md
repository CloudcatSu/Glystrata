...
> **註（0.2.0）：** 本文件是 0.0.1 的需求基線，保留原樣供追溯；之後的行為變更以 `CHANGELOG.md` 為準。與目前版本主要不同之處：
>
> - 快照 sidecar 預設為一般可見檔案，可在「偏好設定 → 快照」選擇隱藏；檔案開頭附有給 AI／自動化工具的說明欄位。另有「立即建立快照」手動快照。
> - 群組只加入檔案，不再提供加入資料夾；側邊欄只顯示平面群組名稱，不展開檔案／資料夾項目。
> - 所有視窗改用跟隨主題的自訂標題列（保留 Aero Snap 與 Snap Layouts），工具列只有新增、開啟、左右／上下分割、關閉窗格與還原單一窗格。
> - 分割時把目前窗格的最後一個 TAB 移到新窗格；同一檔案可用 TAB 右鍵「在新窗格開啟」在另一個窗格編輯。TAB 尚不支援拖曳排序或拖到其他窗格。
