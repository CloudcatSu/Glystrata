...
- 新增「尋找與取代」對話框（尋找、取代、上一個／下一個、全部取代），並在對話框新增「尋找」按鈕，位於「上一個／下一個」左側。
- 分頁重新命名、分頁與群組的拖曳排序、分頁拖曳移動到群組。
- Heptabase 風格的 Markdown 打字助手，以及 Markdown 語法說明頁。
- 快照按鈕永遠可點擊、支援在檔案總管顯示，快照 sidecar 改用 `.gss` 副檔名並可自動從舊命名遷移。
- 第二次啟動程式時，會把要開啟的檔案轉交給既有視窗，並註冊 `.gss` 快照檔的圖示。
- 閱讀器顏色可自訂（含自訂 HSV 色彩選擇器）。
- Enter／Esc 對話框慣例，下拉選單整格皆可點擊展開。

### Changed

- 偏好設定「編輯器分色」更名為「編輯器選項」；「閱讀器顏色」更名為「閱讀器選項」，並整合原本獨立的「渲染閱讀器頁面」設定（標題字級、行距、分段間隔等）。
- 編輯選單的「尋找」「取代」合併為單一「尋找與取代」項目（原本就是同一個對話框，只是聚焦欄位不同）。
- 捲軸寬度加寬 50%（8px → 12px）。

### Fixed

- 修正 Setext 標題（以 `===`／`---` 底線標示的標題）吃掉下一段落內容的問題。
- 修正視窗層級的拖放事件處理：不論拖曳的內容是否為外部檔案，都會把事件標記為已處理，導致分頁／群組拖曳排序全部失效。現在只在真的有外部檔案拖入時才攔截，其餘一律讓內部的拖曳邏輯自行處理。
- 修正編輯區域無法用滑鼠拖曳選取文字的問題。編輯器是分頁（`TabItem`）的 `Content`，在 WPF 中這會讓編輯器成為分頁的「邏輯子項目」，因此整個編輯區域的滑鼠事件也會繞經分頁本身。分頁拖曳排序的處理器掛在整個分頁上，於是在編輯區域按住拖曳時會被判定成「開始拖曳分頁」，進而由拖放迴圈接管滑鼠，AvalonEdit 再也收不到滑鼠移動事件，選取範圍自然畫不出來。現在只有真正從分頁本身（視覺子樹）開始的拖曳才會啟動分頁拖曳。
- 舊版（MDeditor 時期）快照 sidecar 的遷移改為啟動時全面掃描既有檔案，不再只在使用者實際打開快照紀錄時才觸發。

## [1.1.1] - 2026-09-15

### Fixed

- 功能列與眼睛圖示在部分電腦（尤其是非高 DPI 螢幕）上會顯示破碎、斷線。原始線稿的線條粗細只佔圖示寬度約 2.6%，縮小畫到 18px 的工具列按鈕時線寬不到 1 個裝置像素，縮放結果會因螢幕的 DPI 設定而不同，導致外框、線段隨機消失。改為加粗所有工具列圖示與眼睛圖示的線條，縮小顯示時仍能穩定畫出完整線條。
- 舊版本（專案改名為 Glystrata 之前，當時還叫 MDeditor）產生的快照 sidecar 檔名是 `.<檔名>.mdeditor-snapshots.json`，改名後的版本只認得 `.glystrata-snapshots.json`，導致這些舊快照完全消失。現在開啟文件時若找不到新檔名但找到舊檔名，會自動搬移成新檔名並繼續使用。
