...
- 修正在縮排的清單項目按 Enter 時，下一行會再多縮一層的問題。編輯器本身在換行時就已經沿用上一行的縮排，清單延續又把同樣的縮排加了一次；現在改為取代而不是疊加，新項目會與上一行維持同一層。
- 修正在偏好設定修改群組色票後，已經使用那個顏色的群組不會跟著更新、必須重新選一次的問題。群組存的是顏色值本身而非色票編號，現在按下確定時會依色票位置把新顏色帶到使用它的群組，側邊欄與分頁立即更新；被刪除的色票上的群組則保留原本的顏色。
- 修正狀態列上方的分隔線只畫到編輯區、沒有延伸到群組面板底下的問題。這條線原本是編輯窗格的下框線，現在改由狀態列自己畫，橫跨整個視窗寬度。

## [1.2.0] - 2026-09-19

### Added

- 主題新增「跟隨系統」，並設為預設值。切換 Windows 的淺色／深色時會即時跟著變，不需重開程式。
- 群組顏色：在群組上按右鍵即可指定，選單每一項顯示色塊與色碼。顏色會以直槓顯示在群組名稱左側，以及該群組底下每個文件分頁的檔名左側；切到「所有文件」時，每個分頁仍顯示各自所屬群組的顏色。
- 偏好設定新增「群組顏色」分頁，可新增、修改、刪除色票（預設五色），並用滑桿調整「被選取群組的底色濃度」。
- 被選取的群組會以該群組顏色的淡色底色標示（原本的選取小圓點已移除）。
- 偏好設定新增「匯出設定檔」「匯入設定檔」，檔案格式為副檔名 `.gss` 的 JSON。
- 文件分頁的右鍵選單新增「立即建立快照」。
- 快捷鍵新增十組：快照紀錄 `Ctrl+Shift+H`、立即建立快照 `Ctrl+Alt+S`、偏好設定 `Ctrl+,`、左右分割 `Ctrl+\`、上下分割 `Ctrl+Alt+\`、關閉目前窗格 `Ctrl+Shift+W`、還原單一窗格 `Ctrl+Alt+1`、在新檢視開啟 `Ctrl+Alt+N`、開關閱讀器 `Ctrl+Shift+R`、新增群組 `Ctrl+Shift+G`。
- 選單列支援 Alt 助憶鍵（`Alt+F` 開啟「檔案」，其餘依此類推），讓每個選單項目都能只靠鍵盤到達。
- 快照可以寫註解，預設空白，直接在快照紀錄的清單列上就地編輯，離開欄位或關閉視窗時存檔。
- 「檔案」選單新增「最近開啟過的文件」，列出最近十個檔案，滑鼠停留在檔名上會顯示完整路徑。

### Changed

- 「全部TAB」改名為「所有文件」。
- 群組的「加入檔案」改名為「開啟檔案」，而且會真的把檔案開起來。原本這個項目只會把路徑寫進 `groups.json`，而群組節點根本不列出檔案，所以按下去畫面完全沒有反應。
- 字元計數模式（計入或不計入空白）的選項移到偏好設定的「一般」分頁，狀態列那一格改為純顯示結果，不再是按鈕。
- 偏好設定的「還原預設」移到按鈕列左側，匯出／匯入設定檔排在它旁邊。
- 選單文字與快捷鍵改由同一張命令表產生。原本兩者是各自手寫的兩份清單，改了其中一邊而忘記另一邊就會不一致，現在結構上不可能發生。

### Fixed

- 修正第二次啟動程式時，既有視窗不會跳到前景、只有工作列圖示閃動的問題。Windows 只接受「目前擁有前景的行程」交出前景，因此既有視窗自己呼叫 `Activate()` 一定會被拒絕並降級成閃爍。現在改由剛啟動、握有前景權限的那個行程，在結束前直接把既有視窗還原並拉到前面。
- 修正視窗合成層底色在切換主題後沒有跟著更新的問題。
- 修正主題字典把選取項目底色寫死為透明，導致群組選取完全沒有視覺回饋的問題。
- 修正選單裡的分隔線樣式。WPF 會在子選單實體化時改用 `MenuItem.SeparatorStyleKey` 重新套用樣式，而主題字典沒有定義這個鍵，於是掉回 Windows 預設的內縮樣式，和程式其他地方的線粗細與長度都不一致。
- 修正快照的「讀取→修改→寫入」競態。建立、刪除快照與寫入註解都是先讀出整份快照清單、改完再整份寫回，但這段過程沒有任何互斥保護，因此自動快照計時器若剛好和手動操作重疊，後寫入的一方會以過期的清單覆蓋掉先寫入的一方，靜默弄丟一份快照或一則剛打好的註解。現在同一份快照檔的讀取與寫入全程由同一把鎖保護。
