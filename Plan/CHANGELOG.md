...
本檔案記錄 MDeditor 的版本、功能開發、錯誤修正與重要行為變更。
...
- `dotnet build MDeditor.sln -c Debug --no-restore -m:1 -p:UseSharedCompilation=false`：0 warning、0 error。
- `MDeditor.Verification`：所有驗證 assertions 通過。
...
- `dotnet build`、`MDeditor.Verification`：新增字元統計、Markdown 格式化與新設定欄位驗證全部通過。
...
- `dotnet build MDeditor.sln -c Debug --no-restore -m:1 -p:UseSharedCompilation=false`：成功，0 warning、0 error。
- `MDeditor.Verification`：全部核心驗證通過，包含編碼、文件多視圖、群組移動、狀態持久化、Markdown／YAML、快照與差異比較。
- `MDeditor.Verification`：新增巢狀 pane 收合、單窗格還原、重疊存檔與儲存失敗保留內容驗證，全部通過。
- Debug 與 Release self-contained 單檔 EXE 均可建立 `MDeditor` 主視窗；發布目錄包含 `MDeditor.exe` 與 `THIRD-PARTY-NOTICES.txt`。
- 使用者回報的 `MDeditor.exe` 崩潰事件已確認為 .NET `Win32Exception 1816`（WPF `HwndTarget` 更新視窗時的資源配額不足），已加入記錄與非致命處理；錯誤記錄位於 `%LocalAppData%\MDeditor\errors.log`。
